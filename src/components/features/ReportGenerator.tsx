...
import { ArrowRight, Calculator, FileText, RefreshCw } from 'lucide-react';
...
export function ReportGenerator() {
  ...
  return (
    <section className="bg-[var(--wax)] border border-[var(--line)] rounded-xl p-5 md:p-7">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-5">
        <div>
          <p className="kicker mb-2">Pencil-math engine</p>
          ...
          <p className="text-sm text-[var(--ink-soft)] mt-2 max-w-2xl">
            Type in your household size and income — the desk computes your FY2026 AMI band, what a
      ...
      <div className="grid md:grid-cols-[auto_1fr_auto] items-end gap-3">
        <label className="block text-sm font-bold">
          ...
          <select
            ...
            className="mt-1 block w-full md:w-40 px-3 py-2.5 bg-[var(--paper)] border border-[var(--line)] rounded-lg font-mono"
          >
        ...
        <label className="block text-sm font-bold">
          ...
          <input
            ...
            className="mt-1 block w-full px-3 py-2.5 bg-[var(--paper)] border border-[var(--line)] rounded-lg font-mono"
        ...
        <div className="mt-6">
          <div className="grid sm:grid-cols-3 gap-3">
            ...
            <div className="bg-[var(--paper)] border border-[var(--line)] rounded-xl p-4">
              ...
              <p className="font-display text-3xl font-black text-[var(--charcoal)] mt-1">{result.desk?.listingsNearBand ?? '—'}</p>
              <p className="text-xs text-[var(--ink-soft)] mt-1">
                {result.desk?.openWaitlists ?? '—'} taking applications right now · {result.desk?.foodSitesInDirectory} food sites · {result.desk?.legalAidOrgs} legal-aid orgs in the directory.
              </p>
            </div>
          </div>
          <div className="mt-4 flex flex-col sm:flex-row sm:items-start justify-between gap-3 bg-[var(--parchment)] border border-[var(--line)] rounded-xl p-4">
